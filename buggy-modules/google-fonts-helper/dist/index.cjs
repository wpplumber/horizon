...
class Downloader extends hookable.Hookable {
  ...
  async execute() {
    ...
    if (overwriting) {
      node_fs.rmSync(outputDir, { recursive: true, force: true });
    }
    await this.callHook("download-css:before", this.url);
    const cssContent = await ofetch.ofetch(this.url, { headers });
...
function parseFontsFromCss(content, fontsPath) {
  ...
  while ((match1 = re.face.exec(content)) !== null) {
    ...
    while ((match2 = re.url.exec(fontface)) !== null) {
      ...
      const ext = node_path.extname(urlPathname);
      if (ext.length < 2) {
        continue;
      }
      const filename = node_path.basename(urlPathname, ext) || "";
